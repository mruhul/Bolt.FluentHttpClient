...
namespace Bolt.FluentHttpClient.Tests
{
    public class UnitTest1 : IClassFixture<FluentHttpClientFixture>
    {
        ...
        {
            await _fixture.Request("/api/books/reset/").GetAsync<string>();
            ...
            var s = await client.PostAsync("http://localhost:58252/api/books", new StringContent(JsonConvert.SerializeObject(bookinput, Formatting.None), Encoding.UTF8, "application/json"));

            s.StatusCode.ShouldBe(HttpStatusCode.Created);

            var addResponse = await _fixture
                .Client
                .Url("http://localhost:55556/api/values")
                .Retry(1)
                .TimeoutInMilliseconds(500)
                .PostAsync<Book, Book>(bookinput);

            addResponse.StatusCode.ShouldBe(HttpStatusCode.Created);
            addResponse.Headers.FirstOrDefault(x => x.Name.IsSame("location")).Value.ShouldBe("/api/books/1");
        }
    }
}
